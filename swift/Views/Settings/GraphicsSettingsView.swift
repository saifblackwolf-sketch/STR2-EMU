  ...
  var body: some View {
    Form {
      Section("Renderer") {
        ...
        Text(
          "Metal is recommended. Software is slow but accurate. Null disables rendering (for testing). Requires restart."
        )
        .font(.caption)
      ...
      Section("Upscaling") {
        ...
        Text(
          "Higher values improve visual quality but reduce performance significantly. 4x+ recommended for A17 Pro / M-series. Requires restart."
        )
        .font(.caption)
        .foregroundStyle(.secondary)
      }
      ...
      Section("Filtering") {
        ...
        Toggle("Mipmapping", isOn: $settings.mipmap)
      ...
      Section("VSync") {
        Stepper(
          "Queue Size: \(settings.vsyncQueueSize)", value: $settings.vsyncQueueSize, in: 2...16)

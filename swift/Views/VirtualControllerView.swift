...
struct VirtualControllerView: View {
  ...
  private func deviceScale(_ geo: GeometryProxy) -> CGFloat {
    ...
    let w =
      isLandscape ? max(geo.size.width, geo.size.height) : min(geo.size.width, geo.size.height)
  ...
  func portraitLayout(w: CGFloat, h: CGFloat) -> some View {
    ZStack {
      // A002: apply opacity to background too
      Color(white: 0.10).opacity(Double(settings.padOpacity))
      ...
      GeometryReader { cGeo in
        ...
        PadBtn(label: "SEL", w: 42, h: 22, btn: .select)
          ...
          .position(
            x: pos("select", landscape: false).x * cW, y: pos("select", landscape: false).y * cH)
        PadBtn(label: "START", w: 48, h: 22, btn: .start)
          .scaleEffect(pos("start", landscape: false).scale)
          .position(
            x: pos("start", landscape: false).x * cW, y: pos("start", landscape: false).y * cH)
        DPadView(size: 100)
          .scaleEffect(pos("dpad", landscape: false).scale)
          .position(
            x: pos("dpad", landscape: false).x * cW, y: pos("dpad", landscape: false).y * cH)
        ActionButtonsView(size: 42)
          .scaleEffect(pos("action", landscape: false).scale)
          .position(
            x: pos("action", landscape: false).x * cW, y: pos("action", landscape: false).y * cH)
        StickView(isLeft: true)
          .scaleEffect(pos("lstick", landscape: false).scale)
          .position(
            x: pos("lstick", landscape: false).x * cW, y: pos("lstick", landscape: false).y * cH)
        StickView(isLeft: false)
          .scaleEffect(pos("rstick", landscape: false).scale)
          .position(
            x: pos("rstick", landscape: false).x * cW, y: pos("rstick", landscape: false).y * cH)
      }
      .opacity(Double(settings.padOpacity))
    }
  }
...
struct PSBtn: View {
  ...
  var body: some View {
    Text(sym)
      ...
      .background(
        Circle().fill(on ? clr.opacity(0.6) : .black.opacity(0.25))
...
struct StickView: View {
  ...
  var body: some View {
    ZStack {
      Circle().fill(.black.opacity(0.18)).stroke(.white.opacity(0.18), lineWidth: 1).frame(
        width: sz)
      ...
      DragGesture(minimumDistance: 0)
        .onChanged { v in
          ...
          if dist > 4 {
            ...
            let nx = Float(cos(a) * d / maxR)
            let ny = Float(sin(a) * d / maxR)

            var finalNy = ny
            if isLeft && SettingsStore.shared.invertVirtualLeftStickY { finalNy = -finalNy }
            if !isLeft && SettingsStore.shared.invertVirtualRightStickY { finalNy = -finalNy }

            isLeft
              ? EmulatorBridge.shared.setLeftStick(x: nx, y: finalNy)
              : EmulatorBridge.shared.setRightStick(x: nx, y: finalNy)
          }
        }
        .onEnded { _ in
          if !isDragging {

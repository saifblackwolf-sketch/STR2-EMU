...
final class PadLayoutStore: @unchecked Sendable {
  ...
  static let groupIDs = [
    "dpad", "action", "l1", "l2", "r1", "r2", "lstick", "rstick", "select", "start",
  ]
  ...
  func save() {
    for id in Self.groupIDs {
      if let pos = portrait[id] {
        ...
        iPSX2Bridge.setINIFloat(
          "iPSX2/PadLayout/Portrait", key: "\(id)_scale", value: Float(pos.scale))
      }
      if let pos = landscape[id] {
        ...
        iPSX2Bridge.setINIFloat(
          "iPSX2/PadLayout/Landscape", key: "\(id)_scale", value: Float(pos.scale))
      }
    }
  }
  ...
  func load() {
    for id in Self.groupIDs {
      ...
      if px >= 0 {
        let py = iPSX2Bridge.getINIFloat(
          "iPSX2/PadLayout/Portrait", key: "\(id)_y", defaultValue: 0.5)
        let ps = iPSX2Bridge.getINIFloat(
          "iPSX2/PadLayout/Portrait", key: "\(id)_scale", defaultValue: 1.0)
        portrait[id] = PadGroupPosition(x: CGFloat(px), y: CGFloat(py), scale: CGFloat(ps))
      }
      // Landscape
      let lx = iPSX2Bridge.getINIFloat(
        "iPSX2/PadLayout/Landscape", key: "\(id)_x", defaultValue: -1)
      if lx >= 0 {
        let ly = iPSX2Bridge.getINIFloat(
          "iPSX2/PadLayout/Landscape", key: "\(id)_y", defaultValue: 0.5)
        let ls = iPSX2Bridge.getINIFloat(
          "iPSX2/PadLayout/Landscape", key: "\(id)_scale", defaultValue: 1.0)

...
+ (void)setPaused:(BOOL)paused {
    ...
    NSLog(@"[iPSX2Bridge] VM paused state set to: %d", paused);
}

+ (BOOL)saveState:(nonnull NSString *)path {
    if (VMManager::GetState() == VMState::Shutdown) return NO;
    bool success = VMManager::SaveState([path UTF8String], true, false);
    if (success) {
        NSLog(@"[iPSX2Bridge] Successfully triggered save state at %@", path);
    } else {
        NSLog(@"[iPSX2Bridge] Failed to trigger save state at %@", path);
    }
    return success;
}

+ (BOOL)loadState:(nonnull NSString *)path {
    if (VMManager::GetState() == VMState::Shutdown) return NO;
    bool success = VMManager::LoadState([path UTF8String]);
    if (success) {
        NSLog(@"[iPSX2Bridge] Successfully triggered load state from %@", path);
    } else {
        NSLog(@"[iPSX2Bridge] Failed to trigger load state from %@", path);
    }
    return success;
}
...
+ (void)logUserMarker:(nonnull NSString *)name {
    Log::Write(LOGLEVEL_INFO, Color_StrongCyan, "@@@ USER_MARKER: [%s] @@@", [name UTF8String]);
}

+ (void)triggerGSDump {
    // Single frame dump
    MTGS::RunOnGSThread([]() { GSQueueSnapshot(std::string(), 1); });
    Log::Write(LOGLEVEL_INFO, Color_StrongYellow, "@@@ GS_DUMP_TRIGGERED @@@");
}

...
extern SettingsInterface* g_p44_settings_interface;
...
// [iPSX2] Log Limiting Logic (moved to Global scope/Log Namespace)
    ...
    namespace Log {
		static void ExecuteCallbacks(LOGLEVEL level, ConsoleColors color, std::string_view message)
        {
			...
			// --- LogServer integration: send all logs to remote server ---
			if (g_p44_settings_interface && g_p44_settings_interface->GetBoolValue("LogServer", "Enabled", false)) {
				static bool logserver_initialized = false;
				if (!logserver_initialized) {
					std::string host = g_p44_settings_interface->GetStringValue("LogServer", "Host", "127.0.0.1");
					int port = g_p44_settings_interface->GetIntValue("LogServer", "Port", 5050);
					LogServer::Init(host.c_str(), static_cast<uint16_t>(port));
					logserver_initialized = true;
				}
				static const char* level_names[] = {"NONE", "ERROR", "WARNING", "INFO", "DEV", "DEBUG", "TRACE"};
				std::string logline = std::string("[") + level_names[(int)level] + "] " + std::string(message) + "\n";

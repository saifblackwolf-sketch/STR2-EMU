...
DarwinMisc::JitMode DarwinMisc::DetectJitMode()
{
    ...
    bool cs_debugged = (cs_flags & 0x10000000u);

    if (cs_debugged) {
        // [P52] If launched via StikDebug, use dual-mapping for high performance.
        s_jit_mode = JitMode::LuckTXM;
    } else {
        // [iPSX2] Force Legacy mode for standard side-loading.
...
bool DarwinMisc::IsJITAvailable()
{
    ...
    return true;
#elif TARGET_OS_IPHONE
    // [iPSX2] Bypass strict CS_DEBUGGED check.
    // Logs show "Legacy mprotect OK" even when CS_DEBUGGED=0, so we allow JIT path.
    // This allows the recompiler to run on non-jailbroken devices via side-loading 
    // if they have the proper entitlements or if the environment allows mprotect(RX).
    uint32_t cs_flags = 0;
    int rv = csops(getpid(), 0, &cs_flags, sizeof(cs_flags));
    bool cs_debugged = (rv == 0) && (cs_flags & 0x10000000u);

    fprintf(stderr, "@@JIT_DETECT@@ csops=%d cs_flags=0x%08x CS_DEBUGGED=%d -> result=FORCED_AVAILABLE\n",
...
int DarwinMisc::iPSX2_TRACE_EXEC = [](){
    const char* s = getenv("iPSX2_TRACE_EXEC");
    if (!s) s = getenv("SIMCTL_CHILD_iPSX2_TRACE_EXEC");
    int val = (s && s[0] == '1') ? 1 : 0;
    // Note: SettingsStore also writes this to INI, but DarwinMisc initializes early.
    // If enabled, we want a one-shot confirmation log.
    fprintf(stderr, "@@CFG@@ iPSX2_TRACE_EXEC=%d\n", val);
    return val;
}();

...
{ @autoreleasepool {
	...
	m_current_render.encoder = MRCRetain(enc);
		...
		// Log framebuffer and window size to server log
		if (g_gs_renderer) {
			GSVector2i fb_size = g_gs_renderer->GetInternalResolution();
			LogUnified("@@MTL_FB@@ frame=%u win=%ux%u fb=%dx%d\n", (unsigned)FrameNo(),
				(unsigned)m_window_info.surface_width, (unsigned)m_window_info.surface_height,
				(unsigned)fb_size.x, (unsigned)fb_size.y);
		}
...
void GSDeviceMTL::SendHWDraw(GSHWDrawConfig& config, id<MTLRenderCommandEncoder> enc, id<MTLBuffer> buffer, size_t off, bool one_barrier, bool full_barrier)
{
	// [TEMP_DIAG] GS draw profiling — barrier types + GPU timing per 600 vsync
	{
		...
		const u32 cur_vsync = static_cast<u32>(g_perfmon.GetFrame());

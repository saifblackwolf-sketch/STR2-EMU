...
{
	void ensureLoaded();
	/// Clears the in-memory database and resets the load flag so the next
	/// ensureLoaded() call re-reads GameIndex.yaml from disk.
	/// Only call this when the VM is stopped (no game running).
	void reloadDatabase();

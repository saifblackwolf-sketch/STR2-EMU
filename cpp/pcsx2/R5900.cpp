...
int s_tlbmiss_cap = 0; // [iPSX2] Global cap for TLB miss logging and loop detection
...
void eeloadHook()
{
	...
	if (argc) // calls to EELOAD *after* the first one during the startup process will come here
	{
		...
		if (s_elf_osdsys_override && elfname == "rom0:OSDSYS")
		{
			...
			if (!elf_override.empty())
			{
				...
				if (argv1_ptr && argv1_ptr < 0x02000000u)
				{
					// [iPSX2] Safety: limit overwrite
					std::strncpy((char*)PSM(argv1_ptr), new_elfname.c_str(), 63);
					((char*)PSM(argv1_ptr))[63] = '\0';
				}
				elfname = new_elfname;
		...
		if (!EmuConfig.CurrentGameArgs.empty() && elfname == "rom0:PS2LOGO")
		{
			...
			for (int a = 0; a < argc; a++)
			{
				...
				if (arg_ptr && arg_ptr < 0x02000000u) {
			...
			if (arg_ptr && arg_ptr < 0x02000000u) {
				// [iPSX2] Safety: limit overwrite
				std::strncpy((char *)PSM(arg_ptr + arg_len + 1), EmuConfig.CurrentGameArgs.c_str(), 127);
				((char *)PSM(arg_ptr + arg_len + 1))[127] = '\0';
			}
	...
	if (VMManager::Internal::IsFastBootInProgress() && elfname.empty())
	{
		...
		if (!elfname.empty())
		{
			...
			int osdsys_found = 0;
			// [iPSX2] Search in both EELOAD area and likely relocation area
			const u32 search_ranges[][2] = {
				{ EELOAD_START, EELOAD_START + EELOAD_SIZE },
				{ 0x90000, 0xA0000 } // Likely relocation area for newer BIOS
			};

			for (const auto& range : search_ranges) {
				for (u32 addr = range[0]; addr < range[1] - 12; addr += 4) 
				{
					if (!strcmp((char*)PSM(addr), "rom0:OSDSYS"))
					{
						// [iPSX2] Safety: limit overwrite to original string length (11 chars + null)
						std::strncpy((char*)PSM(addr), elfname.c_str(), 11);
						((char*)PSM(addr))[11] = '\0';
						if (osdsys_found == 0) g_osdsys_str = addr;
						osdsys_found++;
					}
				}
			}
			Console.WriteLn("@@P33_OSDSYS_OVERWRITE@@ elfname='%s' (truncated to 11) osdsys_found=%d G_OSDSYS=%08x",
				elfname.c_str(), osdsys_found, g_osdsys_str);
		}
		else
		{
...
void eeloadHook2()
{
	...
	// [iPSX2] Safety: ensure we don't overflow the EELOAD arg buffer (approx 256 bytes)
	if (game_len + 1 + EmuConfig.CurrentGameArgs.length() < 256) {
		memset(PSM(g_osdsys_str + game_len), 0x20, 1);
		std::strncpy((char *)PSM(g_osdsys_str + game_len + 1), EmuConfig.CurrentGameArgs.c_str(), 254 - game_len);
		((char *)PSM(g_osdsys_str + game_len + 1))[254 - game_len] = '\0';
	}
#if DEBUG_LAUNCHARG

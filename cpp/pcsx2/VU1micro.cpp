...
void vu1ExecMicro(u32 addr)
{
	...
	// [AI Probe] Detect camera/vertex math anomalies
	{
		for (int i = 0; i < 32; i++) {
			for (int j = 0; j < 4; j++) {
				float v = VU1.VF[i].F[j];
				if (std::isnan(v) || std::isinf(v) || std::abs(v) > 100000.0f) {
					Log::Write(LOGLEVEL_INFO, Color_StrongRed, "@@@ AI_PROBE: VU1_ANOMALY vf%02d.%c = %.4f (PC: 0x%X) @@@", 
						i, "xyzw"[j], v, VU1.VI[REG_TPC].UL);
					goto next_kick; // Only log once per kick to avoid spam
				}
			}
		}
	}

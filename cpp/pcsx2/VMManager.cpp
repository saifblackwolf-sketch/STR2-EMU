...
// Must happen after BIOS load, depends on BIOS version.
	...
	const bool fast_boot_pref =
		boot_params.fast_boot.has_value() ? boot_params.fast_boot.value() : static_cast<bool>(EmuConfig.EnableFastBoot);
	s_fast_boot_requested =
		(fast_boot_pref || !s_elf_override.empty()) &&
...
void VMManager::WarnAboutUnsafeSettings()
{
	...
	if (!EmuConfig.EnableGameFixes)
	{
		append(ICON_FA_GAMEPAD,
			TRANSLATE_SV("VMManager", "Optional automatic speedhacks are not enabled. Performance or timing in some games may be affected."));
	}

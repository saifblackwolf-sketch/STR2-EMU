...
//#define EXTRA_DEBUG
	...
	// [iPSX2] Detailed Instruction Trace
	if (DarwinMisc::iPSX2_TRACE_EXEC) {
		Console.WriteLn("@@TRACE_EE@@ pc=%08x op=%-10s cyc=%u a0=%08x a1=%08x a2=%08x a3=%08x v0=%08x v1=%08x t0=%08x t1=%08x t2=%08x ra=%08x sp=%08x", 
			pc, opcode.Name, cpuRegs.cycle,
...
static __fi void _doBranch_shared(u32 tar)
{
	...
	if( cpuRegs.branch != 0 )
	{
		...
		intLastBranchTo = tar;
		// [iPSX2] Safety: Ensure branch target is 4-byte aligned.
		// If a game/BIOS jumps to an unaligned address, hardware would drop bits.
		if (tar & 3) {
			static int s_unaligned_cap = 0;
			if (s_unaligned_cap < 5) {
				Console.Warning("@@INTERP_UNALIGNED_BRANCH@@ pc=%08x tar=%08x (auto-masked)", cpuRegs.pc - 4, tar);
				s_unaligned_cap++;
			}
		}
		cpuRegs.pc = tar & ~3u;

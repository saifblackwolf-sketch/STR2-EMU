...
void D2_DBGP(const u8 *inbuffer, u8 *outbuffer, char *message, char *eepc, char *ioppc, char *eecy, char *iopcy){
	...
	switch(in->type){
		case 0x00://ok
			snprintf(line, sizeof(line), "%s/GETCONF",	in->id==0?"CPU":in->id==1?"VU0":"VU1");
			...
			break;
		case 0x02://ok
            snprintf(line, sizeof(line), "%s/2", in->id==0?"CPU":in->id==1?"VU0":"VU1");
			break;
		case 0x04://ok
            snprintf(line, sizeof(line), "%s/GETREG count=%d kind[0]=%d number[0]=%d",
			...
			break;
		case 0x06://ok
            snprintf(line, sizeof(line), "%s/PUTREG count=%d kind[0]=%d number[0]=%d value=%016I64X_%016I64X",
		...
		case 0x08://ok
		{
			snprintf(line, sizeof(line), "%s/RDMEM %08X/%X",
			...
			if ((mem->address & ((1 << mem->align)-1)) ||
				...
				out->result=1;
				strncat(line, " ALIGN ERROR", sizeof(line) - strlen(line) - 1);
				break;
			}
			if (in->h.destination=='I')
				if (iopVirtMemR<void>(mem->address & 0x1FFFFFFF))
					...
					out->result=0x11;
					strncat(line, " ADDRESS ERROR", sizeof(line) - strlen(line) - 1);
					break;
				}
			else
			{
		...
		case 0x0a://ok
		{
			snprintf(line, sizeof(line), "%s/WRMEM %08X/%X",
				in->id==0?"CPU":in->id==1?"VU0":"VU1", mem->address, mem->length);
			const u8* data=(u8*)in+	//kids: don't try this at home! :D
				((sizeof(DECI2_DBGP_HEADER)+sizeof(DECI2_DBGP_MEM)+(1 << mem->align) - 1) & (0xFFFFFFFF << mem->align));
			if (mem->length==4 && *(int*)data==0x0000000D)
				strncat(line, " BREAKPOINT", sizeof(line) - strlen(line) - 1);
			if ((mem->address & ((1 << mem->align)-1)) ||
				(mem->length  & ((1 << mem->align)-1))){
				out->result=1;
				strncat(line, " ALIGN ERROR", sizeof(line) - strlen(line) - 1);
		...
		case 0x10://ok
		{
			snprintf(line, sizeof(line), "%s/GETBRKPT count=%d",
		...
		case 0x12://ok [does not break on iop brkpts]
			snprintf(line, sizeof(line), "%s/PUTBRKPT count=%d",
				in->id==0?"CPU":in->id==1?"VU0":"VU1", in->count);
			out->h.length=sizeof(DECI2_DBGP_HEADER);
			if (in->count>32){
				out->result=1;
				strncat(line, "TOO MANY", sizeof(line) - strlen(line) - 1);
				break;
			}
		...
		case 0x14://ok, [w/o iop]
			snprintf(line, sizeof(line), "%s/BREAK count=%d",
				in->id==0?"CPU":in->id==1?"VU0":"VU1", in->count);
			if (in->h.destination=='I')
				;
			else{
		...
		case 0x16://ok, [w/o iop]
			snprintf(line, sizeof(line), "%s/CONTINUE code=%s count=%d",
		...
		{
			snprintf(line, sizeof(line), "%s/RUN code=%d count=%d entry=0x%08X gp=0x%08X argc=%d",
...
//			threads_array[0].argstring = 0;
		...
		default:
			snprintf(line, sizeof(line), "type=0x%02X code=%d count=%d [unknown]", in->type, in->code, in->count);
	}
	snprintf(message, 1024, "[DBGP %c->%c/%04X] %s", in->h.source, in->h.destination, in->h.length, line);
	snprintf(eepc, 16, "%08X", cpuRegs.pc);
	snprintf(ioppc, 16, "%08X", psxRegs.pc);
	snprintf(eecy, 16, "%d", cpuRegs.cycle);
	snprintf(iopcy, 16, "%d", psxRegs.cycle);
	writeData(outbuffer);
}

...
void D2_NETMP(const u8 *inbuffer, u8 *outbuffer, char *message){
	...
	switch(in->code){
		case 0:
			...
			snprintf(line, sizeof(line), "code=CONNECT");
			for (i=0; i<n; i++){
				snprintf(p, sizeof(p), " %04X/%d", connect[i].protocol, connect[i].priority);
		...
		case 2:
			...
			IOPboot=*(u64*)data;
			snprintf(line, sizeof(line), "code=RESET EE=0x%I64X IOP=0x%I64X", EEboot, IOPboot);
			////////////////////////////hack
			...
			break;
		case 4://[OK]
			snprintf(line, sizeof(line), "code=MESSAGE %s", data);//null terminated by the memset with 0 call
			strncpy(d2_message, (char*)data, sizeof(d2_message) - 1);
			d2_message[sizeof(d2_message) - 1] = '\0';
			writeData(outbuffer);
			break;
		case 6://[ok]
			snprintf(line, sizeof(line), "code=STATUS");
			...
			*(u32*)data=0;//null end the string on a word boundary
			...
			break;
		case 8:
			snprintf(line, sizeof(line), "code=KILL protocol=0x%04X", *(u16*)data);
			writeData(outbuffer);
			break;
		case 10:
			snprintf(line, sizeof(line), "code=VERSION %s", data);
			data=(u8*)out+sizeof(DECI2_NETMP_HEADER);
			strncpy((char*)data, "0.2.0", 6); // Safety: buffer size is unknown but "0.2.0" is small
			data+=strlen("0.2.0");//emu version;)
			out->h.length=data-(u8*)out;
			writeData(outbuffer);
			break;
		default:
			snprintf(line, sizeof(line), "code=%d[unknown] result=%d", in->code, in->result);
			writeData(outbuffer);
	}
	snprintf(message, 1024, "[NETMP %c->%c/%04X] %s", in->h.source, in->h.destination, in->h.length, line);
}

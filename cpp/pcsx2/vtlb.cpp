...
static __ri void vtlb_Miss(u32 addr, u32 mode)
{
	...
	if (Cpu == &intCpu)
	{
		...
		// [iPSX2] Safety: if we are in a heavy TLB miss loop (cap reached),
		// don't cancel the instruction, just let it return 0/ignore.
		// This prevents the "Black Screen" deadlock when kernel handlers are missing.
...
bool vtlb_Core_Alloc()
{
	...
	// [iPSX2] Check if we are debugged (StikDebug active). 
	// Fastmem (4GB reservation) is much more stable with proper entitlements/debugging.
	uint32_t cs_flags = 0;
	...
	if (!cs_debugged) {
		Console.Warning("@@FASTMEM_SKIP@@ Non-debugged iOS device — force-disabling fastmem for stability");
		vtlbdata.fastmem_base = 0;
		EmuConfig.Cpu.Recompiler.EnableFastmem = false;
		s_fastmem_area = nullptr;
	} else {
	...
	s_fastmem_area = SharedMemoryMappingArea::Create(FASTMEM_AREA_SIZE);
#endif
	if (!s_fastmem_area && EmuConfig.Cpu.Recompiler.EnableFastmem)
	{

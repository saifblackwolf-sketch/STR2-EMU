...
// ---------------------------------------------------------------------------
// [iPSX2] OTA GameIndex updater
// Set this to the raw URL of GameIndex.yaml in your GitHub repo.
// e.g. https://raw.githubusercontent.com/YOUR_USERNAME/iPSX2-src-main/main/assets/resources/GameIndex.yaml
// You can also override this at runtime by setting the NSUserDefaults key
// "iPSX2_GameDB_UpdateURL" before the app launches (useful for testing).
// ---------------------------------------------------------------------------
#define iPSX2_GAMEDB_UPDATE_URL \
    @"https://raw.githubusercontent.com/Thiago-Heleno/iPSX2-src/main/assets/resources/GameIndex.yaml"
...
- (void)sceneDidBecomeActive:(UIScene *)scene {
    if ([iPSX2Bridge isVMRunning]) {
        [iPSX2Bridge setPaused:NO];
        Console.WriteLn("[App] sceneDidBecomeActive: VM resumed.");
    }
}

- (void)sceneWillResignActive:(UIScene *)scene {
    if ([iPSX2Bridge isVMRunning]) {
        [iPSX2Bridge setPaused:YES];
        
        NSString *docsPath = [iPSX2Bridge documentsDirectory];
        NSString *sstateDir = [docsPath stringByAppendingPathComponent:@"sstates"];
        [[NSFileManager defaultManager] createDirectoryAtPath:sstateDir withIntermediateDirectories:YES attributes:nil error:nil];
        NSString *savePath = [sstateDir stringByAppendingPathComponent:@"auto_background.p2s"];
        
        [iPSX2Bridge saveState:savePath];
        Console.WriteLn("[App] sceneWillResignActive: VM paused and auto-saved to %s", [savePath UTF8String]);
    }
...
// ---------------------------------------------------------------------------
// [iPSX2] OTA GameIndex updater
// Downloads GameIndex.yaml from GitHub asynchronously on every launch.
// Uses ETag-based conditional GETs to skip unchanged files.
// Saves to Documents/GameIndex.yaml (picked up by the GameDB loader).
// ---------------------------------------------------------------------------
- (void)fetchGameIndexUpdateInBackground {
    // Allow URL override via NSUserDefaults (e.g. for dev/testing)
    NSString* urlString = [[NSUserDefaults standardUserDefaults]
                            stringForKey:@"iPSX2_GameDB_UpdateURL"];
    if (!urlString || urlString.length == 0) {
        urlString = iPSX2_GAMEDB_UPDATE_URL;
    }

    NSURL* url = [NSURL URLWithString:urlString];
    if (!url) {
        fprintf(stderr, "@@GAMEDB_OTA@@ invalid URL, skipping\n");
        return;
    }
    ...
    [request setValue:@"iPSX2/1.0 (iOS GameIndex OTA)" forHTTPHeaderField:@"User-Agent"];

    // Conditional GET: only download if file changed since last fetch
    NSString* etag = [[NSUserDefaults standardUserDefaults] stringForKey:@"iPSX2_GameDB_ETag"];
    if (etag.length > 0) {
        [request setValue:etag forHTTPHeaderField:@"If-None-Match"];
        fprintf(stderr, "@@GAMEDB_OTA@@ sending conditional GET (ETag: %s)\n", [etag UTF8String]);
    } else {
        fprintf(stderr, "@@GAMEDB_OTA@@ no cached ETag, performing full download\n");
    }
    ...
    NSURLSessionDataTask* task = [session dataTaskWithRequest:request
        completionHandler:^(NSData* data, NSURLResponse* response, NSError* error) {
            if (error) {
                fprintf(stderr, "@@GAMEDB_OTA@@ network error: %s\n",
                    [[error localizedDescription] UTF8String]);
                return;
            }

            NSHTTPURLResponse* http = (NSHTTPURLResponse*)response;
            const long status = http.statusCode;

            if (status == 304) {
                // Server confirmed file hasn't changed — nothing to do
                fprintf(stderr, "@@GAMEDB_OTA@@ 304 Not Modified — GameIndex.yaml is up to date\n");
                return;
            }

            if (status != 200 || !data || data.length == 0) {
                fprintf(stderr, "@@GAMEDB_OTA@@ unexpected status %ld (bytes=%zu), aborting\n",
                    status, (size_t)data.length);
                return;
            }

            // Basic sanity check: must start with '#' (YAML comment) or a serial key
            const char* bytes = (const char*)data.bytes;
            if (data.length < 4 || (bytes[0] != '#' && bytes[0] != 'S' && bytes[0] != 'A' && bytes[0] != 'P')) {
                fprintf(stderr, "@@GAMEDB_OTA@@ response doesn't look like valid YAML, aborting\n");
                return;
            }

            // Write atomically to Documents/GameIndex.yaml
            NSArray* paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
            NSString* docs = [paths firstObject];
            NSString* destPath = [docs stringByAppendingPathComponent:@"GameIndex.yaml"];

            NSError* writeError = nil;
            BOOL ok = [data writeToFile:destPath options:NSDataWritingAtomic error:&writeError];
            if (!ok || writeError) {
                fprintf(stderr, "@@GAMEDB_OTA@@ write failed: %s\n",
                    [[writeError localizedDescription] UTF8String]);
                return;
            }

            // Persist the new ETag for the next launch
            NSString* newETag = http.allHeaderFields[@"ETag"];
            if (newETag.length > 0) {
                [[NSUserDefaults standardUserDefaults] setObject:newETag forKey:@"iPSX2_GameDB_ETag"];
                [[NSUserDefaults standardUserDefaults] synchronize];
            }

            fprintf(stderr, "@@GAMEDB_OTA@@ downloaded %zu bytes -> %s\n",
                (size_t)data.length, [destPath UTF8String]);

            // If the VM is not running, reload the in-memory database immediately
            // so the very first game boot in this session uses the fresh data.
            // If the VM IS running (game already started), the update takes effect
            // on the next app launch.
            if (!s_vmThreadActive.load()) {
                GameDatabase::reloadDatabase();
                fprintf(stderr, "@@GAMEDB_OTA@@ in-memory database reloaded\n");
            } else {
                fprintf(stderr, "@@GAMEDB_OTA@@ VM active — update takes effect on next launch\n");
            }
        }];
    [task resume];
}
...
// DYLD Map — debug builds only
    ...
    // [iPSX2] OTA GameIndex updater — runs async, completes well before user taps Play
    [self fetchGameIndexUpdateInBackground];
